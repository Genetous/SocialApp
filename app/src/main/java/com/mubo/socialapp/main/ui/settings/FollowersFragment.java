...
public class FollowersFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rView = v.findViewById(R.id.followers_list);
    ...
    public void getNew(){
        try {
            ...
            if(last_ind<size) {
                ndata = false;
                getFollowers(last_ind);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    void getFollowers(int start) throws JSONException {
        if(ndata)
            fds.clear();
        JSONObject sendObject=new JSONObject();
        sendObject.put("relationName","userRelations");
        sendObject.put("to",userid);
        JSONObject gbr=new JSONObject();
        gbr.put("relationName","userRelations");
        gbr.put("collection_content._id", "$content.follower.follower_user._id");
    ...
    void bind(JSONObject j) throws JSONException {
        ...
        for(int i=0;i<values.length();++i){
            ...
            JSONObject v_content=p.optJSONObject("content");
            boolean selfFollowed=false;
            String f_folowed_id="";
            String f_folower_id="";
            if(v_content!=null) {
                JSONArray follower = v_content.optJSONArray("follower");
                JSONArray followed = v_content.optJSONArray("followed");
                try {
                    f_folowed_id = follower.getJSONObject(0).getJSONArray("follower_followed")
                            .getJSONObject(0).getString("_id");

                }catch (Exception ex){}
                try {
                    f_folower_id = follower.getJSONObject(0).getString("_id");
                }catch (Exception ex){}
                //user bilgilerini al
                if (follower != null) {
                    for (int c = 0; c < follower.length(); ++c) {
                        JSONObject f_user = follower.optJSONObject(c);
                        try {
