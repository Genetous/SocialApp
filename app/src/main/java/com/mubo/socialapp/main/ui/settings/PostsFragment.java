...
public class PostsFragment extends Fragment {
    ...
    public void getNew(){
        try {
            int last_ind=statics.posts_data.size();
            if(last_ind<size) {
                ndata = false;
                getPosts(last_ind);
            }
    ...
    void getPosts(int start) throws JSONException {
        if(ndata)
            statics.posts_data.clear();
        JSONObject sendObject=new JSONObject();
        sendObject.put("relationName","postRelations");
        sendObject.put("content.user.id",userid);
        sendObject.put("from",0);
        sendObject.put("size",20);
    ...
    void bind(JSONObject j) throws JSONException {
        ...
        for(int i=0;i<values.length();++i){
            ...
            statics.posts_data.add(pd);

        }
        if(ndata){
            statics.posts_adaptor=new postAdaptor(getContext(),statics.posts_data);
            statics.posts_adaptor.setClickListener(new postAdaptor.ItemClickListener() {
                @Override
                public void onItemClick(View view, int position) {
                    String tag=view.getTag().toString();
                    if(tag.equals("profile")) {
                        String userid = statics.posts_data.get(position).getId();
                        Intent i = new Intent(getContext(), UserProfile.class);
                        i.putExtra("userid", userid);
                        startActivity(i);
                    }else if(tag.equals("image")){
                        String url = statics.posts_data.get(position).getContent();
                        Intent i = new Intent(getContext(), ShowImage.class);
                        i.putExtra("image", url);
                        startActivity(i);
                    }else if(tag.equals("main")){
                        PostData postData = statics.posts_data.get(position);
                        Intent i = new Intent(getContext(), SinglePost.class);
                        i.putExtra("post", postData);
                        startActivity(i);
                    }else if(tag.equals("like")) {
                        postData = statics.posts_data.get(position);
            ...
            rView.setAdapter(statics.posts_adaptor);
        }else{
            statics.posts_adaptor.notifyDataSetChanged();
        }
        statics.PostAdaptorList[1]=statics.posts_adaptor;
        isLoading=false;
