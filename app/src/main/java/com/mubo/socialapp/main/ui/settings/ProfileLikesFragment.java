...
public class ProfileLikesFragment extends Fragment {
    ...
    public static ProfileLikesFragment newInstance(String userid) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_profile_likes, container, false);
        LinearLayoutManager layoutManager
                = new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false);
        rView = v.findViewById(R.id.posts_list);
    ...
    public void getNew(){
        try {
            int last_ind=statics.postLikes_data.size();
            if(last_ind<size) {
                ndata = false;
                getPosts(last_ind);
            }
    ...
    public void getPosts(int start) throws JSONException {
        if(ndata)
            statics.postLikes_data.clear();
        JSONObject q1=new JSONObject();
        q1.put("relationName","postRelations");
        q1.put("content.postLike.postLike_user._id",userid);
        JSONArray fields = new JSONArray();
        fields.put("collection_content");
        fields.put("content.user");
        q1.put("fields",fields);
        ApiClass ac=new ApiClass();
        if(ndata)
            ac.getRelations(q1,frm,hnd,getActivity());
        else
            ac.getRelations(q1,null,hnd,getActivity());
    }
    ...
    void bind(JSONObject j) throws JSONException {
        ...
        for(int i=0;i<values.length();++i){
            ...
            boolean selfLiked=true;

            PostData pd = new PostData(user_id,username,time,String.valueOf(likeSize),
                    String.valueOf(commentSize),selfLiked,userimage,"",postData,postType,post_id,"");
            statics.postLikes_data.add(pd);

        }
        if(ndata) {
            statics.postLikes_adaptor = new postAdaptor(getContext(), statics.postLikes_data);
            statics.postLikes_adaptor.setClickListener(new postAdaptor.ItemClickListener() {
                @Override
                public void onItemClick(View view, int position) {
                    String tag = view.getTag().toString();
                    if (tag.equals("profile")) {
                        String userid = statics.postLikes_data.get(position).getId();
                        Intent i = new Intent(getContext(), UserProfile.class);
                        i.putExtra("userid", userid);
                        startActivity(i);
                    } else if (tag.equals("image")) {
                        String url = statics.postLikes_data.get(position).getContent();
                        Intent i = new Intent(getContext(), ShowImage.class);
                        i.putExtra("image", url);
                        startActivity(i);
                    } else if (tag.equals("main")) {
                        PostData postData = statics.postLikes_data.get(position);
                        Intent i = new Intent(getContext(), SinglePost.class);
                        i.putExtra("post", postData);
                        startActivity(i);
                    } else if (tag.equals("like")) {
                        postData = statics.postLikes_data.get(position);
            ...
            rView.setAdapter(statics.postLikes_adaptor);
        }else{
            statics.postLikes_adaptor.notifyDataSetChanged();
        }
        statics.PostAdaptorList[2]=statics.postLikes_adaptor;
        isLoading=false;
    }
    PostData postData;
    int val=0;
